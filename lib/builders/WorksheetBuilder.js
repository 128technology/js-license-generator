...
class WorksheetBuilder {
  constructor() {
    this.rows = [];
    this.worksheetHeaders = ['name', 'version', 'licenses', 'text'];
  }
  ...
  setHeaders(headerArray) {
    this.worksheetHeaders = headerArray;
  }
...
class XLSXBuilder extends WorksheetBuilder {
  write(name) {
    const ws = xlsx.utils.json_to_sheet(this.rows, {
      header: this.worksheetHeaders
    });
...
class CSVBuilder extends WorksheetBuilder {
  write(name) {
    const ws = xlsx.utils.json_to_sheet(this.rows, {
      header: this.worksheetHeaders
    });
...
module.exports = { XLSXBuilder, CSVBuilder, WorksheetBuilder };
